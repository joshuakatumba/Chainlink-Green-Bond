...
var configSchema = exports_external.object({
  ...
  confidenceThreshold: exports_external.number()
});
async function onStart(runtime2) {
  const { assetId, confidenceThreshold } = runtime2.config;
  runtime2.log(`\uD83D\uDE80 Starting RWA Verification for Asset: ${assetId}...`);
  const aiResult = {
    success: true,
    assetId,
  ...
  runtime2.log(`✅ AI Verification Completed. Confidence: ${(aiResult.aiConfidence * 100).toFixed(1)}%`);
  if (aiResult.aiConfidence < confidenceThreshold) {
    throw new Error(`❌ Workflow Aborted: Confidence ${aiResult.aiConfidence} is below threshold ${confidenceThreshold}`);
  }
  runtime2.log("✅ Conditions Met: Confidence Threshold Passed.");
  runtime2.log(`\uD83D\uDCE6 Asset ${assetId} is ready for tokenization.`);
  return "Verification Successful";
}
var initWorkflow = (config) => {
  return [
    cre.handler(cre.capabilities.once.trigger(), onStart)
  ];
};
async function main() {
